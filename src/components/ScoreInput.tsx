...
const ScoreInput = () => {
  ...
  const handleDeleteRound = (round: number) => {
    if (window.confirm(`Are you sure you want to delete Round ${round}? This will recalculate all scores.`)) {
      deleteRound(round);
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          {isEditing ? <Edit className="text-blue-600" size={24} /> : 
           isViewingExistingRound ? <Eye className="text-gray-600" size={24} /> :
           <Plus className="text-primary-600" size={24} />}
          <h2 className="text-2xl font-bold text-gray-800">
            {isEditing ? `Edit Round ${editingRound}` : 
             isViewingExistingRound ? `Round ${viewingRound}` :
             `Round ${totalRounds + 1}`}
          </h2>
        </div>
        {isEditing && (
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="space-y-4">
          {players.map((player, index) => {
            const existingScore = existingRound?.scores.find(s => s.playerId === player.id)?.score || 0;
            const isReadOnly = isViewingExistingRound && !isEditing;
            
            return (
              <div key={player.id} className={`p-4 rounded-lg ${isReadOnly ? 'bg-gray-50' : 'bg-gray-50'}`}>
                <label htmlFor={`score-${player.id}`} className="block text-sm font-medium text-gray-700 mb-2">
                  <div className="flex items-center gap-2">
